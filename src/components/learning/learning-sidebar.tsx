                ...
                >
                  <div className="flex items-center gap-2">
                    {lesson.completed ? (
                      ...
                      <CircleDashed className={`h-4 w-4 shrink-0 ${isLessonActive(lesson.title) ? 'text-primary' : ''}`} />
                    )}
                    <span className={`truncate ${isLessonActive(lesson.title) ? 'font-medium' : ''}`}>{lesson.title}</span>
                  </div>
                </Button>
              ))}
            </div>
